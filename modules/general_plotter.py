...
class PlotWindow(QMainWindow):
    def __init__(self, config_dict):
        ...
        self.ani = animation.FuncAnimation(
            self.figure, self.update_plot, interval=self.current_interval, cache_frame_data=False
        )
...
class CodeHighlighter(QSyntaxHighlighter):
    def __init__(self, parent=None, variable_names=None):
        ...
        if variable_names is not None:
            ...
            self._highlighting_rules += [
                (QRegExp(r'\b' + variable + r'\b'), variable_format) for variable in variable_names
            ]
