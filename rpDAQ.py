...
class rpControl:
    ...
    def __init__(self, config_dict):
        ...
        self.osc_reshaped = self.osc_buffer.reshape(
            (2, self.total_samples), order='F'
        )  # order='F' is so that [1,2,3,4,5,6] -> [[1,3,5], [2,4,6]] which is the way the data is sent
    ...
    def osc_to_view(self, view):
        ...
        while bytes_received < self.CUTOFF_SIZE:
            bytes_received += self.socket.recv_into(
                self.cutoff_view[bytes_received:], self.CUTOFF_SIZE - bytes_received
            )
        self.event_count += 1
    ...
    def load_config_from_dict(self, config_dict):
        ...
        if self.acquisition_mode != 'mimoCoRB':
            if self.acquisition_mode not in ACQUISITION_MODES:
                raise ValueError(
                    str(self.acquisition_mode)
                    + " is not a valid acquisition mode. Must be in "
                    + str(ACQUISITION_MODES)
                )
            if self.acquisition_mode != 'infinite':
        ...
        if self.trigger_source not in TRIGGER_SOURCES:
            raise ValueError(
                str(self.trigger_source) + " is not a valid trigger source. Must be in " + str(TRIGGER_SOURCES.keys())
            )
        self.trigger_slope = config_dict['trigger_slope']
        if self.trigger_slope not in TRIGGER_SLOPES:
            raise ValueError(
                str(self.trigger_slope) + " is not a valid trigger slope. Must be in " + str(TRIGGER_SLOPES.keys())
            )
        self.trigger_mode = config_dict['trigger_mode']
        if self.trigger_mode not in TRIGGER_MODES:
            raise ValueError(
                str(self.trigger_mode) + " is not a valid trigger mode. Must be in " + str(TRIGGER_MODES.keys())
            )
        self.trigger_level = config_dict['trigger_level']
        if abs(self.trigger_level) > ADC_RANGE:
            raise ValueError(
                str(self.trigger_level)
                + " is not a valid trigger level. Must be in [-"
                + str(ADC_RANGE)
                + ", "
                + str(ADC_RANGE)
                + "]"
        ...
        if self.total_samples > MAXIMUM_SAMPLES:
            raise ValueError(
                str(self.total_samples)
                + " is not a valid number of total samples. Must be less than "
                + str(MAXIMUM_SAMPLES)
            )
        ...
        if self.start_generator:
            ...
            if self.distribution not in DISTRIBUTIONS:
                raise ValueError(
                    str(self.distribution) + " is not a valid distribution. Must be in " + str(DISTRIBUTIONS.keys())
                )
...
def rp_mimocorb(source_list=None, sink_list=None, observe_list=None, config_dict=None, **rb_info):
    ...
    config_dict['trigger_source'] = trigger_channel
